...
import { Employee, LeaveRequest, JournalArticle, DailySchedule, ChatMessage, Task, Notification, TaskMilestone, TaskComment, CalendarMarking, CalendarMarkings, MarkingType, ActivePage, CustomerFeedback, StaffEvaluation } from '../types';
import { DEFAULT_EMPLOYEES, initialCustomerFeedbacks, DEFAULT_GOOGLE_FORM_URL, initialStaffEvaluations } from '../data';
import CustomerMicrophones from './CustomerMicrophones';
import StaffEvaluationManager from './StaffEvaluationManager';
...
import { mergeLeaveHistories, mergeEmployeesWithPreservedLeaves, syncAllLeavesToCloud } from '../lib/leaveSyncHelper';
...
export const getEmployeeDailySchedules = (emp: Employee): DailySchedule[] => {
  const isCaisseDefault = emp.isCaisse !== undefined 
    ? emp.isCaisse 
    : (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss'));

  if (emp.dailySchedules && Array.isArray(emp.dailySchedules) && emp.dailySchedules.length === 7) {
    return emp.dailySchedules.map(d => ({
      ...d,
      isCaisse: d.isCaisse !== undefined ? d.isCaisse : isCaisseDefault
    }));
  }
...
interface LeaveSchedulerProps {
  ...
  onUpdateFeedbacks?: (feedbacks: CustomerFeedback[]) => void;
...
export default function LeaveScheduler({ 
  ...
  setFeaturedImage,
  onOpenGpsModal,
  customerFeedbacks = initialCustomerFeedbacks,
  onUpdateFeedbacks,
  googleFormUrl = DEFAULT_GOOGLE_FORM_URL,
  onUpdateGoogleFormUrl,
  evaluations = initialStaffEvaluations,
  onUpdateEvaluations = () => {}
}: LeaveSchedulerProps) {
  // Parloir SubTab state (Évaluation vs Discussion Chat)
  const [parloirSubTab, setParloirSubTab] = useState<'evaluations' | 'chat'>('evaluations');

  // State
  const [employees, setEmployees] = useState<Employee[]>(() => {
    let baseList: Employee[] = DEFAULT_EMPLOYEES;
    ...
    // Recover any leave requests stored on this mobile phone
    let localLeaves: any[] = [];
    try {
      const storedLeaves = localStorage.getItem('pharmintl_all_leave_requests');
      if (storedLeaves) {
        const parsedLeaves = JSON.parse(storedLeaves);
        if (Array.isArray(parsedLeaves)) localLeaves = parsedLeaves;
      }
    } catch {}

    const withDaily = baseList.map((emp: any) => {
      const empExtraLeaves = localLeaves.filter(l => l.employeeId === emp.id || l.employeeName === emp.name);
      const mergedLeaves = empExtraLeaves.length > 0
        ? mergeLeaveHistories(emp.leaveHistory || [], empExtraLeaves)
        : (emp.leaveHistory || []);
  ...
  useEffect(() => {
    try {
      if (loggedInUser) {
        localStorage.setItem('pharmintl_logged_in_user', JSON.stringify(loggedInUser));
      } else {
        localStorage.removeItem('pharmintl_logged_in_user');
      }
      // Dispatch custom event for real-time components sync across tabs/components
      window.dispatchEvent(new Event('pharmintl_auth_changed'));
    } catch (e) {
      console.error(e);
    }
  }, [loggedInUser]);
  ...
  const [isScheduleSlicerModalOpen, setIsScheduleSlicerModalOpen] = useState(false);
  ...
  const [editedIsPfQualite, setEditedIsPfQualite] = useState(false);
  ...
  const [newEmpIsPfQualite, setNewEmpIsPfQualite] = useState(false);
  ...
  const saveTasks = (updated: Task[]) => {
    ...
    pushRealtimeUpdate('tasks', 'tasks', updated, ts);
  };
  ...
  const saveNotifications = (updated: Notification[]) => {
    ...
    pushRealtimeUpdate('notifications', 'notifications', updated, ts);
  };
  ...
  const saveCalendarMarkings = (updated: CalendarMarkings) => {
    ...
    pushRealtimeUpdate('calendar_markings', 'markings', updated, ts);
  };
  ...
  // Setup instant real-time sync (SSE + BroadcastChannel) and cloud Firestore listeners
  useEffect(() => {
    // 1. Instant Real-Time Multi-User & Multi-Tab Synchronization Engine
    const unsubRealtime = registerRealtimeSync({
      onEmployees: (data, incomingTime = 0) => {
        if (Array.isArray(data) && data.length > 0) {
          setEmployees(current => {
            const merged = mergeEmployeesWithPreservedLeaves(current, data);
            const withDaily = merged.map((emp: any) => ({
              ...emp,
              dailySchedules: emp.dailySchedules || getEmployeeDailySchedules(emp)
            }));
            localStorage.setItem('pharmintl_employees', JSON.stringify(withDaily));
            localStorage.setItem('pharmintl_employees_updated_at', String(incomingTime || Date.now()));
            return withDaily;
          });
        }
      },
      onLeaveRequests: (incomingLeaves, incomingTime = 0) => {
        if (Array.isArray(incomingLeaves) && incomingLeaves.length > 0) {
          setEmployees(current => {
            const updated = current.map(emp => {
              ...
              const empIncoming = incomingLeaves.filter((r: any) => {
              ...
              if (empIncoming.length === 0) return emp;
              const mergedLeaves = mergeLeaveHistories(emp.leaveHistory || [], empIncoming);
              return { ...emp, leaveHistory: mergedLeaves };
            });
      ...
      onNotifications: (data, incomingTime = 0) => {
      ...
      onOwnerPassword: (password, incomingTime = 0) => {
        const localTime = parseInt(localStorage.getItem('pharmintl_owner_password_updated_at') || '0', 10);
        if (incomingTime >= localTime && typeof password === 'string' && password.trim()) {
          setOwnerPassword(password.trim());
          localStorage.setItem('pharmintl_owner_password', password.trim());
          localStorage.setItem('pharmintl_owner_password_updated_at', String(incomingTime));
        }
      }
    });

    // Initial push of all local leave requests from this device to cloud & server
    syncAllLeavesToCloud(employees);

    // 2. Cloud Firestore Listeners as additional cloud backup
      ...
      (data, updatedAt) => {
        if (data && Array.isArray(data) && data.length > 0) {
          setEmployees(current => {
            ...
            localStorage.setItem('pharmintl_employees', JSON.stringify(withDaily));
            localStorage.setItem('pharmintl_employees_updated_at', String(updatedAt || Date.now()));
            return withDaily;
          });
        }
      },
    ...
    // 2b. Cloud Firestore Leave Requests listener
    const unsubLeaveRequests = subscribeToDoc<any[]>(
      'leave_requests',
      'leaveRequests',
      (data) => {
        if (data && Array.isArray(data) && data.length > 0) {
          setEmployees(current => {
            const updated = current.map(emp => {
              ...
              const empIncoming = data.filter((r: any) => {
                const rId = String(r.employeeId || r.userId || '').toLowerCase();
                const rName = String(r.employeeName || r.userName || '').toLowerCase();
                return (rId && (rId === empId || rId === empUsername)) || (rName && rName === empName);
              });
      ...
      () => {
        try {
          const storedLeaves = localStorage.getItem('pharmintl_all_leave_requests');
          if (storedLeaves) return JSON.parse(storedLeaves);
        } catch {}
        return [];
      }
    );
    ...
    const unsubChat = subscribeToDoc<ChatMessage[]>(
      ...
      (data, updatedAt) => {
        ...
        if (incomingTime >= localTime && data && Array.isArray(data)) {
          const sorted = [...data].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
          setChatMessages(sorted);
          localStorage.setItem('pharmintl_chat_messages', JSON.stringify(sorted));
          localStorage.setItem('pharmintl_chat_messages_updated_at', String(incomingTime));
  ...
  // Calendar edit mode toggle (active for supervisor only)
  ...
  const saveToStorage = (updated: Employee[]) => {
    ...
    pushRealtimeUpdate('employees', 'employees', updated, ts);
    syncAllLeavesToCloud(updated);
  };
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (userClean === 'elhaidinam@gmail.com') {
      ...
      if (passClean === ownerPassword) {
        ...
        onPageChange('workforce'); // auto transition to Workforce space
        setLoginUsername('');
        setLoginPassword('');
    ...
    if (foundEmp) {
      setLoggedInUser({
        ...
        name: foundEmp.name,
        isPfQualite: !!foundEmp.isPfQualite,
        isSupervisor: !!foundEmp.isSupervisor
      });
  ...
  // Action: Toggle Caisse assignment checkbox for employee (globally or for a filtered day)
  const handleToggleEmployeeCaisse = (empId: string, dayName?: string) => {
    const updated = employees.map(emp => {
      if (emp.id === empId) {
        const daily = getEmployeeDailySchedules(emp);
        if (dayName && dayName !== 'Tous') {
          const currentDay = daily.find(d => d.dayName === dayName);
          const currentDayCaisse = currentDay?.isCaisse !== undefined 
            ? currentDay.isCaisse 
            : (emp.isCaisse ?? (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss')));
          const nextDayCaisse = !currentDayCaisse;

          const dailyUpdated = daily.map(d => {
            if (d.dayName === dayName) {
              return { ...d, isCaisse: nextDayCaisse };
            }
            return d;
          });

          const hasAnyCaisse = dailyUpdated.some(d => d.isCaisse);
          return {
            ...emp,
            isCaisse: hasAnyCaisse,
            dailySchedules: dailyUpdated
          };
        } else {
          // Global employee caisse toggle
          const currentCaisse = emp.isCaisse !== undefined 
            ? emp.isCaisse 
            : (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss'));
          const nextCaisse = !currentCaisse;
          const dailyUpdated = daily.map(d => ({
            ...d,
            isCaisse: nextCaisse
          }));
  ...
  const handleStartEditingCredentials = (emp: Employee) => {
    ...
    setEditedIsPfQualite(emp.isPfQualite || false);
    ...
    return (
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        <div className="border-b border-gray-100 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Visual day count indicator & Supervisor Edit button */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Button to edit weekly schedule - Active for supervisors only */}
            {(loggedInUser?.type === 'supervisor' || loggedInUser?.type === 'owner') ? (
              <button
                type="button"
                onClick={() => setIsScheduleSlicerModalOpen(true)}
                className="px-3.5 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-900 border border-orange-200 rounded-2xl text-xs font-black shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
                title="Accéder au panneau de configuration des plannings hebdomadaires"
              >
                <Clock size={13} className="text-orange-600" />
                <span>✏️ Éditer l'emploi du temps</span>
              </button>
            ) : (
              <button
                type="button"
                disabled
                className="px-3.5 py-1.5 bg-gray-100 text-gray-400 border border-gray-200 rounded-2xl text-xs font-black opacity-60 cursor-not-allowed flex items-center gap-1.5"
                title="Réservé aux superviseurs et à la direction"
              >
                <Lock size={12} />
                <span>🔒 Éditer l'emploi du temps (Superviseurs uniquement)</span>
              </button>
            )}
          </div>
        </div>
        ...
        {employees.filter(emp => emp.isScheduleApproved).length === 0 ? (
          ...
          /* Table View for 'Tous les jours' */
          <div className="overflow-x-auto rounded-3xl border border-gray-200/80 bg-white shadow-xs">
            <table className="w-full text-left border-collapse min-w-[850px]">
              <thead>
                <tr className="bg-gradient-to-r from-emerald-800 to-teal-800 text-white text-xs uppercase font-extrabold tracking-wider">
                  <th className="p-4 border-b border-emerald-900 min-w-[200px] sticky left-0 bg-emerald-800 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.15)]">
                  ...
                  {['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'].map((dayName) => (
                    <th
                      key={dayName}
                      onClick={() => setSelectedSlicerDay(dayName)}
                      className="p-3.5 text-center border-b border-emerald-900 hover:bg-emerald-700/60 transition-all cursor-pointer min-w-[110px]"
                      title={`Filtrer sur ${dayName}`}
                    >
              ...
              <tbody className="divide-y divide-gray-100 text-xs font-medium">
                {[...employees]
                  ...
                  .map((emp) => {
                    ...
                    const isEmpCaisse = emp.isCaisse ?? (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss'));
                    return (
                      <tr key={emp.id} className="hover:bg-emerald-50/20 transition-colors group">
                        {/* Employé (Tête de ligne avec case à cocher Caisse) */}
                        <td className="p-3.5 sticky left-0 bg-white group-hover:bg-emerald-50/40 border-r border-gray-100 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] transition-colors">
                          <div className="flex items-center gap-2.5">
                            {/* Case à cocher Caisse */}
                            <label 
                              className={`flex items-center gap-1.5 px-2 py-1 rounded-xl border transition-all cursor-pointer select-none shadow-2xs shrink-0 ${
                                isEmpCaisse
                                  ? 'bg-amber-500 text-white border-amber-600 font-black'
                                  : 'bg-gray-50 hover:bg-gray-100 text-gray-700 border-gray-200 font-bold'
                              }`}
                              title={isEmpCaisse ? "Assigné à la Caisse (cliquer pour modifier)" : "Non assigné à la Caisse (cliquer pour cocher)"}
                              onClick={(e) => {
                                e.stopPropagation();
                                handleToggleEmployeeCaisse(emp.id);
                              }}
                            >
                              <input 
                                type="checkbox" 
                                checked={!!isEmpCaisse}
                                onChange={() => {}}
                                className="w-3.5 h-3.5 rounded text-amber-600 focus:ring-amber-500 border-gray-300 cursor-pointer accent-amber-600"
                              />
                              <span className="text-[10px] uppercase">Caisse</span>
                            </label>

                            <img
                              src={emp.avatar}
                              alt={emp.name}
                              className="w-8 h-8 rounded-full border border-gray-200 shadow-xs object-cover shrink-0"
                            />
                            <div className="min-w-0">
                              <div className="flex items-center gap-1">
                                <h5 className="font-extrabold text-xs text-gray-900 truncate">{emp.name}</h5>
                                {isEmpCaisse && (
                                  <span className="text-[8px] bg-amber-100 text-amber-900 border border-amber-300 px-1 rounded font-black shrink-0">
                                    Caisse
                                  </span>
                                )}
                              </div>
                              <p className="text-[10px] text-gray-500 font-semibold truncate">{emp.role}</p>
                            </div>
                          </div>
                        </td>

                        {/* Horaires en cellules */}
                        {dailySchedules.map((day) => (
                          <td
                            key={day.dayName}
                            onClick={() => setSelectedSlicerDay(day.dayName)}
                            className="p-3 text-center align-middle border-r border-gray-100 last:border-r-0 hover:bg-emerald-50/40 transition-colors cursor-pointer"
                            title={`Voir les détails pour ${day.dayName}`}
                          >
                            {day.isActive ? (
                              <div className="space-y-1">
                                <div className="inline-block bg-[#EDFCE8] text-emerald-900 border border-green-200 px-2.5 py-1 rounded-xl font-mono font-black text-xs shadow-2xs">
                                  {day.startHour}h-{day.endHour}h
                                </div>
                                {day.isActive2 && (
                                  <div className="text-[10px] font-mono font-bold text-emerald-700">
                                    / {day.startHour2}h-{day.endHour2}h
                                  </div>
                                )}
                                {day.isActive3 && (
                                  <div className="text-[10px] font-mono font-bold text-emerald-700">
                                    / {day.startHour3}h-{day.endHour3}h
                                  </div>
                                )}
                                <div className="flex items-center justify-center gap-1">
                                  <div className="text-[9px] font-bold text-emerald-700 uppercase">Actif</div>
                                  {day.isCaisse && (
                                    <span className="text-[8px] bg-amber-100 text-amber-800 border border-amber-200 px-1 py-0.2 rounded font-black">
                                      Caisse
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="space-y-0.5 opacity-50">
                                <span className="inline-block bg-gray-100 text-gray-400 border border-gray-200/60 px-2 py-0.5 rounded-lg text-[10px] font-bold italic">
                                  Repos
                                </span>
                              </div>
                            )}
          ...
          /* Detailed Single-Day Filter View with Interactive Time Bars / Cursors */
          <div className="space-y-4">
            <div className="bg-emerald-50/80 border border-emerald-200/80 p-4 rounded-2xl flex items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <span className="text-emerald-700 text-sm font-black">📅 Jour filtré : {selectedSlicerDay}</span>
                <span className="text-xs text-gray-500 font-semibold">(Affichage avec curseurs horaires)</span>
              </div>
              <button
                type="button"
                onClick={() => setSelectedSlicerDay('Tous')}
                className="px-3 py-1.5 bg-white hover:bg-gray-100 text-emerald-800 border border-emerald-200 rounded-xl text-xs font-black shadow-2xs transition-all cursor-pointer flex items-center gap-1.5"
              >
                <span>← Voir tous les jours (Tableau)</span>
              </button>
            </div>
            ...
            {(() => {
              ...
              return sortedActiveEmployees.map((emp) => {
                const daily = getEmployeeDailySchedules(emp);
                const daySched = daily.find(d => d.dayName === selectedSlicerDay)!;
                const isCaisseChecked = daySched.isCaisse !== undefined 
                  ? daySched.isCaisse 
                  : (emp.isCaisse ?? (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss')));
                ...
                return (
                  <div 
                    key={emp.id}
                    className={`p-5 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-5 border transition-all shadow-xs ${
                      isCaisseChecked
                        ? 'bg-amber-50/60 border-amber-200'
                        : 'bg-[#EDFCE8]/40 border-green-100'
                    }`}
                  >
                    {/* Left Profile details with Caisse Checkbox in front of employee */}
                    <div className="flex items-center gap-3 w-full md:w-[35%] shrink-0">
                      {/* Case à cocher Caisse devant chaque personnel */}
                      <label 
                        className={`flex items-center gap-2 px-3 py-2 rounded-2xl border transition-all cursor-pointer select-none shadow-xs shrink-0 ${
                          isCaisseChecked
                            ? 'bg-amber-500 text-white border-amber-600 shadow-amber-200'
                            : 'bg-white hover:bg-gray-100 text-gray-700 border-gray-300'
                        }`}
                        title={isCaisseChecked ? "Collaborateur assigné à la Caisse (cliquer pour décocher)" : "Cliquer pour assigner ce personnel à la Caisse"}
                        onClick={(e) => {
                          e.stopPropagation();
                          handleToggleEmployeeCaisse(emp.id, selectedSlicerDay);
                        }}
                      >
                        <input 
                          type="checkbox" 
                          checked={!!isCaisseChecked}
                          onChange={() => {}} 
                          className="w-4 h-4 rounded text-amber-600 focus:ring-amber-500 border-gray-300 cursor-pointer accent-amber-600"
                        />
                      ...
                      <img 
                        src={emp.avatar} 
                        alt={emp.name} 
                        className="w-10 h-10 rounded-full border border-gray-200 bg-white shadow-xs object-cover shrink-0"
                      />
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <h5 className="font-extrabold text-xs text-gray-900 truncate">{emp.name}</h5>
                          {isCaisseChecked && (
                            <span className="text-[9px] bg-amber-100 text-amber-900 border border-amber-300 px-1.5 py-0.2 rounded font-black shrink-0">
                              En caisse
                            </span>
                          )}
                        </div>
                        <p className="text-[10px] text-gray-500 font-semibold truncate">{emp.role}</p>
                        <span className="inline-block text-[9px] px-2 py-0.5 rounded-lg font-bold font-mono mt-1 leading-tight bg-green-100 text-green-800 border border-green-200">
                    ...
                    {/* Right interactive daily timeline bar (Curseurs horaires de programmation) */}
    ...
    return (
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4">
          ...
          {/* Year Selector & Supervisor Edit Button */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Calendar Edit Button - Active for supervisors only */}
            {isSupervisorOnly ? (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  ...
                  title="Activer ou désactiver le mode d'édition du calendrier"
                >
                  <Edit3 size={13} />
                  <span>{isCalendarEditing ? '✏️ Mode Édition Actif' : '✏️ Éditer le calendrier'}</span>
                </button>
              ...
              <button
                type="button"
                disabled
                className="px-3.5 py-1.5 rounded-xl text-xs font-black bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed opacity-60 flex items-center gap-1.5"
                title="L'édition du calendrier est réservée aux superviseurs et à la direction"
              >
                <Lock size={12} />
                <span>🔒 Éditer le calendrier (Superviseurs uniquement)</span>
              </button>
            )}
          </div>
        </div>
        ...
        {isSupervisorOnly && isCalendarEditing && (
                  ...
                  {cells.map((dayCell, dayCellIdx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => {
                          if (isSupervisorOnly && isCalendarEditing) {
  ...
  useEffect(() => {
    if (activePage === 'private') {
      onPageChange('workforce');
      const timer = setTimeout(() => {
        const el = document.getElementById('private-space-section');
        if (el) {
  ...
  const handleGoToPersonalSection = () => {
    if (!loggedInUser) {
      setShowLoginModal(true);
      return;
    }
    if (activePage !== 'workforce') {
      onPageChange('workforce');
    }
    setTimeout(() => {
      const el = document.getElementById('private-space-section');
  ...
  const renderNavigationBar = () => {
    return (
      <div className="w-full py-0.5">
        {/* Single-line Page Buttons & Extreme-Right Auth */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 w-full">
          {/* Left / Navigation Page Buttons (Stretched full-width flex-1) */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1 w-full">
            <button
              onClick={() => onPageChange('home')}
              className={`flex-1 min-w-0 px-2 sm:px-3 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer text-center ${
                activePage === 'home'
                  ? 'bg-[#22c55e] text-white shadow-md shadow-green-200'
                  : 'bg-white/80 hover:bg-white text-gray-800 border border-green-200/60'
              }`}
            >
              🚀 Accueil
            </button>
            ...
            <button
              ...
              className={`flex-1 min-w-0 px-2 sm:px-3 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 text-center ${
                !loggedInUser
                  ...
                  : 'bg-white/80 hover:bg-white text-gray-800 border border-green-200/60 cursor-pointer'
              }`}
              title={!loggedInUser ? "Accès au Journal Officiel réservé aux utilisateurs connectés" : "Accéder au Journal Officiel"}
            >
              📰 Journal
              {!loggedInUser && <span className="text-[9px] ml-0.5">🔒</span>}
            </button>

            <button
              onClick={() => onPageChange('micros_clients')}
              className={`flex-1 min-w-0 px-2 sm:px-3 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer text-center ${
                activePage === 'micros_clients'
            ...
            <button
              ...
              title={!loggedInUser ? "Accès au Parloir réservé aux utilisateurs connectés" : "Accéder à l'espace Parloir"}
            >
              💬 Parloir
              {!loggedInUser && <span className="text-[9px] ml-0.5">🔒</span>}
            </button>
          ...
          {/* Extreme Right: Login / Personal Space Button */}
          <div className="flex items-center gap-2 shrink-0 ml-auto pl-2 border-l border-gray-200/80">
            {loggedInUser ? (
              <div className="flex items-center gap-1.5">
                {/* Notification Bell */}
                <div className="relative shrink-0">
                  <button
                    ...
                    className={`relative p-1.5 rounded-xl border transition-all hover:scale-105 active:scale-95 flex items-center justify-center cursor-pointer ${
                  ...
                  >
                    <Bell size={14} />
                    {unreadCount > 0 && (
                      <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] font-black h-3.5 w-3.5 rounded-full flex items-center justify-center animate-pulse">
                        {unreadCount}
                      </span>
                    )}
                  </button>
                ...
                <button
                  onClick={handleGoToPersonalSection}
                  className="px-3 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200"
                >
                  👤 {loggedInUser.name}
                </button>
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm hover:shadow-md active:scale-95 shrink-0"
              >
                <Lock size={12} /> Connexion
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Unified Main Content Area based on activePage */}
      <AnimatePresence mode="wait">
        <motion.div
          key={activePage}
        ...
        >
          ...
          {/* =========================================================================
              🚀 ACCUEIL (HOME) PAGE - CALENDRIER ANNUEL
              ========================================================================= */}
          {activePage === 'home' && (
            <div id="home-annual-calendar" className="space-y-4">
              {renderAnnualCalendar()}
            </div>
          )}

          {/* =========================================================================
              🎤 MICROS CLIENTS PAGE - SLIDO LIVE PRESENTATION & GOOGLE FORM FEEDBACKS
              ========================================================================= */}
          {activePage === 'micros_clients' && (
            <div id="micros-clients-section" className="space-y-6">
              <CustomerMicrophones
                feedbacks={customerFeedbacks}
                onUpdateFeedbacks={(updated) => {
                  if (onUpdateFeedbacks) {
                    onUpdateFeedbacks(updated);
                  }
                }}
                googleFormUrl={googleFormUrl}
                onUpdateGoogleFormUrl={onUpdateGoogleFormUrl}
                isSupervisorOrOwner={loggedInUser?.type === 'supervisor' || loggedInUser?.type === 'owner'}
              />
            </div>
          )}

            {/* 2. Section: Work Force (LIST EMPLOYEES + PROFILE & LEAVES + WEEKLY SCHEDULE) */}
            {activePage === 'workforce' && (
              <div className="space-y-10">
                {/* 2.1 Effectif & Biographies Grid */}
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                  <div className="border-b border-gray-100 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <h4 className="text-xl font-extrabold text-gray-800 flex items-center gap-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Découvrez nos collaborateurs engagés. Cliquez sur un profil pour afficher son historique et sa biographie.
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      {/* Bouton Pointer maintenant officiel (identique à la dallette Accueil) */}
                      <button
                        type="button"
                        onClick={() => setIsAttendanceModalOpen(true)}
                        className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl text-xs font-black shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer shrink-0 active:scale-95"
                        title="Enregistrer un pointage (Arrivée / Départ)"
                      >
                        <LogIn size={15} />
                        <span>Pointer maintenant</span>
                      </button>

                      {/* Slicer de Planification (superviseurs/owner) */}
                      {(loggedInUser?.type === 'supervisor' || loggedInUser?.type === 'owner') && (
                        <button
                          type="button"
                          onClick={() => setIsScheduleSlicerModalOpen(true)}
                          className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-xs font-black shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer shrink-0"
                          title="Accéder au Slicer de Planification de l'Emploi du Temps Hebdomadaire"
                        >
                          <Sliders size={16} />
                          <span>Slicer de Planification</span>
                        </button>
                      )}

                      {/* Administration des Utilisateurs (Actif uniquement pour elhaidinam@gmail.com, grisé sinon) */}
                      {(() => {
                        const isElhaidinamAdmin = Boolean(
                          loggedInUser && (
                            loggedInUser.username?.toLowerCase() === 'elhaidinam@gmail.com' ||
                            (loggedInUser as any).email?.toLowerCase() === 'elhaidinam@gmail.com' ||
                            (loggedInUser as any).userId?.toLowerCase() === 'elhaidinam@gmail.com'
                          )
                        );
                        ...
                        return (
                          <button
                            type="button"
                            disabled={!isElhaidinamAdmin}
                            onClick={() => {
                              if (isElhaidinamAdmin) {
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 shrink-0 ${
                              isElhaidinamAdmin
                                ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg cursor-pointer active:scale-95'
                                : 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed opacity-75'
                            }`}
                            title={
                              isElhaidinamAdmin
                                ? "Accéder à la plateforme d'administration des utilisateurs"
                                : "Administration des Utilisateurs réservée à elhaidinam@gmail.com"
                            }
                          >
                            <Users size={16} />
                      ...
                      <div className="text-[11px] bg-emerald-50 text-emerald-800 font-extrabold px-3 py-1.5 rounded-xl border border-emerald-100 shrink-0">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {[...employees].sort(compareEmployeesByLastName).map((emp) => (
                      <button
                        ...
                        className="group text-left p-4 bg-gray-50 hover:bg-green-50/50 rounded-2xl border border-gray-200/80 hover:border-green-300 transition-all duration-300 hover:shadow-md flex flex-col items-center text-center space-y-3 relative cursor-pointer"
                      >
                        <div className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm rounded-full p-1 border border-gray-200 group-hover:bg-green-100 group-hover:text-green-700 transition-colors">
                ...
                {/* 2.2 Emploi du temps hebdomadaire (Weekly Schedule) - Visible uniquement pour celui qui est connecté */}
                <div id="workforce-weekly-timetable" className="space-y-4">
                  {loggedInUser ? (
                    ...
                    <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm text-center space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-500 max-w-md mx-auto">
                          L'emploi du temps hebdomadaire est réservé aux collaborateurs connectés. Veuillez vous connecter pour consulter les plannings et horaires de présence.
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setShowLoginModal(true)}
                        className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black transition-all cursor-pointer shadow-sm active:scale-95 inline-flex items-center gap-2"
                      >
                        <Lock size={13} />
                        <span>Se connecter pour voir l'emploi du temps</span>
                      </button>
                    </div>
                  )}
                </div>

                {/* 2.3 Historique et Traçabilité des Pointages (visible uniquement aux utilisateurs connectés) */}
                {loggedInUser && (
                ...
                {(loggedInUser?.type === 'supervisor' || loggedInUser?.type === 'owner') && (
                  <div id="planning-space-section" className="space-y-8 pt-6 border-t-2 border-emerald-100">
                    <>
                      ...
                      {/* Feature 1: Leave Approval Form (Table or cards of pending leaves) */}
                      {/* End of Supervisor Space */}
                    </>
                  </div>
                )}

                {/* 2.4 Espace Personnel (Uniquement quand connecté) */}
                {loggedInUser && (
                  <div id="private-space-section" className="space-y-8 pt-8 border-t-2 border-emerald-100">
                    {/* If Logged In as Owner without specific employee card */}
                    {loggedInUser.type === 'owner' && !currentLoggedInEmployee ? (
                      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 bg-amber-50 text-amber-700 rounded-2xl flex items-center justify-center font-black text-xl border border-amber-200 shadow-sm">
                            👑
                          </div>
                          <div>
                        ...
                        <button
                          onClick={handleLogout}
                          className="px-5 py-2.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-extrabold text-xs rounded-xl transition-all flex items-center gap-2 cursor-pointer shrink-0"
                        >
                          <LogOut size={14} />
                          Déconnexion
                        </button>
                      ...
                      /* If Logged In as Employee or Owner with Employee profile */
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex items-center gap-4">
                              <div className="relative group shrink-0">
                                <img 
                                  ...
                                  className="w-16 h-16 rounded-full border-2 border-emerald-600 shadow-md bg-white object-cover"
                                />
                                <button
                                  type="button"
                                  onClick={() => setShowAvatarEditor(!showAvatarEditor)}
                                  className="absolute -bottom-1 -right-1 bg-emerald-600 hover:bg-emerald-700 text-white p-1.5 rounded-full shadow-lg border border-white transition-all scale-95 hover:scale-105 flex items-center justify-center cursor-pointer"
                              ...
                              <div className="space-y-1">
                                <span className="bg-emerald-600 text-white text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider block w-fit">
                                  👤 Mon Espace Collaborateur Actif
                                </span>
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                              <button
                                onClick={handleLogout}
                                className="px-5 py-2.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-extrabold text-xs rounded-xl transition-all flex items-center gap-2 cursor-pointer"
                              >
                                <LogOut size={14} />
                                Se déconnecter
                              </button>
                          ...
                          {showAvatarEditor && (
                            <div className="bg-emerald-50 border border-emerald-200 p-6 rounded-3xl space-y-4 shadow-inner">
                              <div className="flex items-center justify-between border-b border-emerald-200 pb-3">
                                <h4 className="text-xs font-black uppercase text-emerald-950 flex items-center gap-2">
                                  ...
                                  Personnaliser ma Photo de Profil / Avatar
                                </h4>
                                <button 
                              ...
                              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
                                {[
                                  ...
                                  'https://images.unsplash.com/photo-1594824813566-7885a3961ad2?auto=format&fit=crop&q=80&w=300',
                                  ...
                                  <button
                                    ...
                                    className={`relative rounded-full overflow-hidden border-2 transition-all p-0.5 hover:scale-105 cursor-pointer ${
                                      currentLoggedInEmployee.avatar === url 
                                        ? 'border-emerald-600 ring-2 ring-emerald-400' 
                                        : 'border-white hover:border-emerald-300'
                                    }`}
                                  >
                                    <img src={url} alt={`Avatar option ${i+1}`} className="w-12 h-12 rounded-full object-cover" />
                                  </button>
                                ))}
                              </div>
                            </div>
                          ...
                          {/* Leave Accounts (Allocated, Consumed, Remaining) */}
                          ...
                          {/* Leave Submission Form & History */}
                          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            {/* Leave Request Form */}
                            <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                              <h4 className="text-base font-extrabold text-gray-800 flex items-center gap-2 border-b pb-3">
                              ...
                              <form onSubmit={handleSubmitLeave} className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    ...
                                    <input
                                      ...
                                      className="w-full text-xs p-2.5 rounded-xl border border-gray-300 bg-gray-50 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500"
                                      required
                                    />
                                  </div>
                                ...
                                <button
                                  type="submit"
                                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl shadow-md uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer"
                                >
                                  <Send size={14} />
                                  Envoyer ma demande
                                </button>
                            ...
                            <div className="lg:col-span-7 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                              <h4 className="text-base font-extrabold text-gray-800 flex items-center gap-2 border-b pb-3">
                                ...
                                Historique de mes demandes de congé ({currentLoggedInEmployee.leaveHistory.length})
                              </h4>

                              {currentLoggedInEmployee.leaveHistory.length === 0 ? (
                                <div className="text-center py-10 text-gray-400 text-xs italic bg-gray-50 rounded-xl border border-dashed">
                                  Vous n'avez soumis aucune demande de congé pour le moment.
                                </div>
                              ) : (
                                  ...
                                  <table className="w-full text-left text-xs">
                                    <thead>
                                      <tr className="border-b border-gray-100 text-[10px] text-gray-400 uppercase font-black">
                                        ...
                                        <th className="py-2.5 px-3 text-center">Jours</th>
                                        <th className="py-2.5 px-3 text-right">Statut</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-50">
                                      {currentLoggedInEmployee.leaveHistory.map((leave) => (
                            ...
                            <form onSubmit={handleSaveBio} className="space-y-4">
                              <div>
                                ...
                                <textarea
                                  ...
                                  rows={4}
                                  placeholder="Rédigez une courte biographie présentant votre rôle et votre parcours à l'officine..."
                                  className="w-full text-xs p-3 rounded-2xl border border-gray-300 bg-gray-50 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500 leading-relaxed"
                                  maxLength={500}
                                />
            ...
            {activePage === 'tasks' && (
              <div id="public-tasks-section" className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                <TaskManager
                  ...
                  viewMode={(loggedInUser?.type === 'supervisor' || loggedInUser?.type === 'owner') ? 'supervisor' : 'public'}
            ...
            {/* Public Section: Parloir (Evaluation & Discussion Room) */}
            {activePage === 'chat' && (
              <div id="public-parloir-section" className="space-y-6">
                {/* Parloir Navigation Sub-tabs */}
                <div className="bg-white/90 backdrop-blur-md p-2 md:p-3 rounded-2xl border border-emerald-100 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                      id="tab-parloir-evaluation"
                      onClick={() => setParloirSubTab('evaluations')}
                      className={`flex-1 sm:flex-initial px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 cursor-pointer ${
                        parloirSubTab === 'evaluations'
                          ? 'bg-emerald-700 text-white shadow-md shadow-emerald-200'
                          : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                    >
                      <Award className="w-4 h-4 text-amber-300" />
                      Évaluation & Points Staff
                    </button>
                    <button
                      id="tab-parloir-chat"
                      onClick={() => setParloirSubTab('chat')}
                      className={`flex-1 sm:flex-initial px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 cursor-pointer ${
                        parloirSubTab === 'chat'
                          ? 'bg-emerald-700 text-white shadow-md shadow-emerald-200'
                          : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    ...
                    >
                      ...
                      Canal de Discussion (Chat)
                    </button>
                  </div>

                  <div className="text-xs text-gray-500 font-bold px-3 py-1 bg-gray-50 rounded-xl border border-gray-100 hidden md:block">
                    Page Parloir — Pharmacie Internationale
                  </div>
                </div>

                {/* Sub-tab 1: Évaluation du Personnel */}
                {parloirSubTab === 'evaluations' && (
                  <StaffEvaluationManager
                    employees={employees}
            ...
            {/* 5. Section: Espace Personnel / Connexion (Déplacé en section 2.4 de la page Work Force) */}
            {false && (
              <div id="private-space-section" className="space-y-8">
            ...
            {!loggedInUser ? (
                ...
                <form onSubmit={handleLogin} className="p-6 space-y-4">
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    <button
                      ...
                      className="w-full py-2.5 px-3 bg-purple-50 hover:bg-purple-100 text-purple-900 border border-purple-200 rounded-xl font-extrabold text-xs shadow-sm flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >
                      <Radio className="w-4 h-4 text-purple-600 animate-pulse" />
                      <span>Extraire GPS & Coordonnées de Connexion</span>
                    </button>
                  </div>
                </form>
              </div>
              ...
              loggedInUser.type === 'owner' ? (
                <div className="space-y-8">
                  ...
                  {/* Private Space Tool: GPS & Device Audit */}
                  <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-purple-800/40">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center shrink-0 border border-white/20">
                        <Radio className="w-7 h-7 text-purple-300 animate-pulse" />
                      </div>
                      <div className="space-y-1">
                        <span className="bg-purple-800/80 text-purple-200 text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                          📍 Outil d'Audit Sécurisé
                        </span>
                        ...
                        <p className="text-xs text-purple-200 max-w-xl">
                          Extraire automatiquement vos coordonnées GPS précises, la date, l'heure, l'identifiant, l'adresse IP et les caractéristiques de votre terminal de connexion.
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={onOpenGpsModal}
                      className="w-full md:w-auto px-6 py-3.5 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white font-extrabold text-xs rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer shrink-0 border border-purple-400/30 active:scale-95"
                    >
                      <MapPin className="w-4 h-4 text-purple-200" />
                      <span>Lancer l'Extraction GPS</span>
                    </button>
                  </div>
                    ...
                    <div className="xl:col-span-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                      ...
                      <form onSubmit={handleCreateEmployee} className="space-y-3.5 text-xs">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                          ...
                          <div className="flex items-center gap-2 bg-emerald-50/70 p-2.5 rounded-xl border border-emerald-200">
                            <input
                              type="checkbox"
                              id="newEmpIsPfQualite"
                              checked={newEmpIsPfQualite}
                              onChange={(e) => setNewEmpIsPfQualite(e.target.checked)}
                              className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                            />
                            <label htmlFor="newEmpIsPfQualite" className="text-[10px] font-black text-emerald-900 uppercase cursor-pointer">
                              🏅 PF qualité
                            </label>
                          </div>
                            ...
                            {employees.map((emp) => {
                              ...
                              return (
                                <tr key={emp.id} className="hover:bg-gray-50/30">
                                  ...
                                  {/* isSupervisor & isPfQualite Habilitations */}
                                  <td className="p-3 text-center">
                                    {isEditing ? (
                                      <div className="flex flex-col gap-1.5 items-center justify-center">
                                        <div className="flex items-center gap-1.5 bg-orange-50 px-2 py-1 rounded-lg border border-orange-200 w-full justify-center">
                                          <input
                                            type="checkbox"
                                        ...
                                        <div className="flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-200 w-full justify-center">
                                          <input
                                            type="checkbox"
                                            id={`edit-pfqualite-${emp.id}`}
                                            checked={editedIsPfQualite}
                                            onChange={(e) => setEditedIsPfQualite(e.target.checked)}
                                            className="rounded text-emerald-600 focus:ring-emerald-500 w-3.5 h-3.5 cursor-pointer"
                                          />
                                          <label htmlFor={`edit-pfqualite-${emp.id}`} className="text-[10px] font-black text-emerald-900 cursor-pointer">
                                            PF qualité
                                          </label>
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex flex-col items-center justify-center gap-1">
                                        ...
                                        {emp.isPfQualite && (
                                          <span className="bg-emerald-100 text-emerald-900 border border-emerald-300 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider">
                                            🏅 PF qualité
                                          </span>
                                        )}
                                        {!emp.isSupervisor && !emp.isPfQualite && (
                    ...
                    <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-purple-800/40">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-purple-200 max-w-xl">
                            Extraire automatiquement vos coordonnées GPS précises, la date, l'heure, votre identifiant, l'adresse IP et les caractéristiques de votre terminal (Téléphone, Tablette ou Ordinateur).
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        <MapPin className="w-4 h-4 text-purple-200" />
                        <span>Extraire mes coordonnées GPS</span>
                      </button>
                    </div>
      ...
      {/* 🔐 Login Modal (Accessible from top bar) */}
      <AnimatePresence>
        {showLoginModal && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
              ...
              className="max-w-md w-full bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden relative"
            >
              <button
                type="button"
              ...
              <form onSubmit={handleLogin} className="p-6 space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl shadow-lg uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  <Lock size={14} />
                ...
                <div className="pt-2 text-center">
                  ...
                  >
                    Mot de passe administrateur / propriétaire perdu ?
                  </button>
                </div>
              </form>
            </motion.div>
      ...
      {/* 👥 User Administration Modal (Accessible from Work Force section) */}
      <AnimatePresence>
        {isUserAdminModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 md:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              className="bg-white rounded-3xl max-w-6xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 shadow-2xl border border-gray-200 relative space-y-6 my-auto"
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-extrabold text-gray-900 tracking-tight">
                      Plateforme d'Administration des Utilisateurs
                    </h3>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Gérez les comptes, ajoutez de nouveaux collaborateurs, modifiez leurs accès ou supprimez des utilisateurs.
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setIsUserAdminModalOpen(false)}
              ...
              {/* Bento Grid: Form + Table */}
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                {/* Left: Register New Employee (xl:col-span-4) */}
                <div className="xl:col-span-4 bg-gray-50/50 p-6 rounded-3xl border border-gray-200/80 shadow-sm space-y-4">
                  ...
                  <form onSubmit={handleCreateEmployee} className="space-y-3.5 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        value={newEmpName}
                        onChange={(e) => setNewEmpName(e.target.value)}
                        className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white font-semibold"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          ...
                          value={newEmpUsername}
                          onChange={(e) => setNewEmpUsername(e.target.value)}
                          className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white font-semibold text-blue-600"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase mb-1">Mot de passe *</label>
                        <input
                          ...
                          value={newEmpPassword}
                          onChange={(e) => setNewEmpPassword(e.target.value)}
                          className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white font-mono"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                      <div className="flex items-center gap-2 bg-white p-2.5 rounded-xl border border-gray-200">
                        <input
                          type="checkbox"
                          id="modalNewEmpIsSupervisor"
                          checked={newEmpIsSupervisor}
                          onChange={(e) => setNewEmpIsSupervisor(e.target.checked)}
                          className="w-4 h-4 rounded text-orange-600 focus:ring-orange-500 cursor-pointer"
                        />
                        <label htmlFor="modalNewEmpIsSupervisor" className="text-[10px] font-black text-gray-700 uppercase cursor-pointer">
                          🛡️ Superviseur
                        </label>
                      </div>
                      ...
                      <div className="flex items-center gap-2 bg-emerald-50/70 p-2.5 rounded-xl border border-emerald-200">
                        <input
                          type="checkbox"
                          id="modalNewEmpIsPfQualite"
                          checked={newEmpIsPfQualite}
                          onChange={(e) => setNewEmpIsPfQualite(e.target.checked)}
                          className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                        />
                        <label htmlFor="modalNewEmpIsPfQualite" className="text-[10px] font-black text-emerald-900 uppercase cursor-pointer">
                          🏅 PF qualité
                        </label>
                      </div>
                      ...
                      <div>
                        <input
                          ...
                          value={newEmpAllocatedLeaves}
                          onChange={(e) => setNewEmpAllocatedLeaves(parseInt(e.target.value) || 0)}
                          className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white font-semibold text-center"
                          placeholder="Congés alloués (ex: 30)"
                        />
                      </div>
                    </div>
                    ...
                    <button
                      type="submit"
                      className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs rounded-xl shadow-md uppercase tracking-wider transition-colors cursor-pointer"
                    >
                      Enregistrer le Collaborateur
                    </button>
                  </form>
                ...
                {/* Right: Register of Accounts and Credentials table (xl:col-span-8) */}
                <div className="xl:col-span-8 bg-white p-6 rounded-3xl border border-gray-200/80 shadow-sm space-y-4">
                  <div>
                    <h4 className="text-base font-extrabold text-gray-800 flex items-center gap-2">
                      <Edit3 className="w-5 h-5 text-blue-600" />
                      Registre Actif des Collaborateurs & Habilitations
                                ...
                                {isEditing ? (
                                  <div className="flex flex-col gap-1.5 items-center justify-center">
                                    <div className="flex items-center gap-1.5 bg-orange-50 px-2 py-1 rounded-lg border border-orange-200 w-full justify-center">
                                      <input
                                        type="checkbox"
                                        id={`modal-edit-super-${emp.id}`}
                                        checked={editedIsSupervisor}
                                        onChange={(e) => setEditedIsSupervisor(e.target.checked)}
                                        className="rounded text-orange-600 focus:ring-orange-500 w-3.5 h-3.5 cursor-pointer"
                                      />
                                      <label htmlFor={`modal-edit-super-${emp.id}`} className="text-[10px] font-black text-orange-800 cursor-pointer">
                                        Superviseur
                                      </label>
                                    </div>
                                    <div className="flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-200 w-full justify-center">
                                      <input
                                        type="checkbox"
                                        id={`modal-edit-pfqualite-${emp.id}`}
                                        checked={editedIsPfQualite}
                                        onChange={(e) => setEditedIsPfQualite(e.target.checked)}
                                        className="rounded text-emerald-600 focus:ring-emerald-500 w-3.5 h-3.5 cursor-pointer"
                                      />
                                      <label htmlFor={`modal-edit-pfqualite-${emp.id}`} className="text-[10px] font-black text-emerald-900 cursor-pointer">
                                        PF qualité
                                      </label>
                                    </div>
                                  </div>
                                ...
                                {isEditing ? (
                                  <div className="flex gap-1 justify-end">
                                    <button
                                      type="button"
                                      onClick={() => handleSaveCredentials(emp.id)}
                                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[9px] px-2 py-1 rounded shadow cursor-pointer"
                                    >
                                      Sauver
                                    </button>
                                    <button
                                      type="button"
                                      onClick={() => setEditingEmployeeId(null)}
                                      className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-[9px] px-2 py-1 rounded cursor-pointer"
                                    >
                                      Annuler
                                    </button>
                                  </div>
                                  ...
                                  <div className="flex gap-1 justify-end items-center">
                                    <button
                                      type="button"
                                      onClick={() => handleStartEditingCredentials(emp)}
                                      className="bg-blue-50 hover:bg-blue-100 text-blue-800 font-extrabold text-[10px] px-2.5 py-1 rounded flex items-center gap-1 shadow-sm transition-all cursor-pointer"
                                      title="Éditer le profil et les accès"
                                    >
                                      <Edit3 size={10} /> Éditer
                                    </button>
                                    <button
                                      type="button"
                                      onClick={() => handleRevokeEmployee(emp.id, emp.name)}
                                      className="bg-red-50 hover:bg-red-100 text-red-600 font-extrabold text-[10px] px-2.5 py-1 rounded flex items-center gap-1 shadow-sm transition-all cursor-pointer"
                                      title="Révoquer / Supprimer le compte"
                                    >
                                      <X size={11} /> Révoquer
                                    </button>
              ...
              <div className="flex justify-end border-t border-gray-100 pt-4">
                <button
                  type="button"
                  onClick={() => setIsUserAdminModalOpen(false)}
                  className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-black rounded-xl transition-all cursor-pointer"
                >
                  Fermer
                </button>
              </div>
      ...
      {/* 📅 Schedule Planning Slicer Modal (Accessible for Supervisors & Owners) */}
      <AnimatePresence>
        {isScheduleSlicerModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 md:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            >
              <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                ...
                <button
                  type="button"
                  onClick={() => setIsScheduleSlicerModalOpen(false)}
                  className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-colors cursor-pointer"
                  title="Fermer"
                >
                  <X size={20} />
                  ...
                  return (
                    ...
                    >
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                        {/* Name Card with Caisse checkbox */}
                        <div className="lg:col-span-3 flex items-center gap-3">
                          <label 
                            className={`flex items-center gap-1.5 px-2 py-1 rounded-xl border transition-all cursor-pointer select-none shadow-2xs shrink-0 ${
                              (emp.isCaisse ?? (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss')))
                                ? 'bg-amber-500 text-white border-amber-600 font-black'
                                : 'bg-white hover:bg-gray-100 text-gray-700 border-gray-300 font-bold'
                            }`}
                            title="Assigner / Retirer de la Caisse"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleToggleEmployeeCaisse(emp.id);
                            }}
                          >
                            <input 
                              type="checkbox" 
                              checked={!!(emp.isCaisse ?? (typeof emp.role === 'string' && emp.role.toLowerCase().includes('caiss')))}
                              onChange={() => {}}
                              className="w-3.5 h-3.5 rounded text-amber-600 focus:ring-amber-500 border-gray-300 cursor-pointer accent-amber-600"
                            />
                            <span className="text-[10px] uppercase">Caisse</span>
                          ...
                          <div className="min-w-0">
                            <h5 className="font-extrabold text-sm text-gray-900 truncate">{emp.name}</h5>
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 space-y-4 shadow-inner">
                              ...
                              <div className="space-y-3.5 divide-y divide-gray-100">
                                {dailySchedules.map((day, idx) => (
                                  ...
                                  >
                                    {/* Left: day name + toggle active checkbox + Caisse toggle */}
                                    <div className="flex items-center gap-2 md:w-1/4">
                                      <span className="text-xs font-black text-gray-800 w-20 flex items-center gap-1">
                                      ...
                                      <button
                                        ...
                                        className={`px-2.5 py-1.5 rounded-xl font-black text-[9px] uppercase transition-all flex items-center gap-1 ${
                                          day.isActive 
                                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' 
                                            : 'bg-gray-100 text-gray-400 border border-gray-200'
                                        }`}
                                      >
                                        {day.isActive ? "🟢 Actif" : "⚪ Repos"}
                                      </button>
                                      <label
                                        className={`flex items-center gap-1 px-2 py-1 rounded-xl border text-[9px] font-black uppercase cursor-pointer select-none transition-all shadow-2xs ${
                                          day.isCaisse
                                            ? 'bg-amber-500 text-white border-amber-600'
                                            : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                        }`}
                                        title={`Assigner à la caisse le ${day.dayName}`}
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          handleToggleEmployeeCaisse(emp.id, day.dayName);
                                        }}
                                      >
                                        <input
                                          type="checkbox"
                                          checked={!!day.isCaisse}
                                          onChange={() => {}}
                                          className="w-3 h-3 rounded text-amber-600 focus:ring-amber-500 border-gray-300 cursor-pointer accent-amber-600"
                                        />
                                        <span>Caisse</span>
                                      </label>
                                    </div>
      ...
      {/* 📍 Modal de Pointage Officiel (Arrivée / Départ horodaté & géolocalisé) */}
      <AttendanceModal
